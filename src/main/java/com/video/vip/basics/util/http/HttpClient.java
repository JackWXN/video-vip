...
public class HttpClient {
     ...
     * 最大不要超过1000
     */
    private static int maxConnTotal = 300;
    /**
    ...
    public static void main(String args[]) {
        try {
           /*声明封装类 默认请求是get*/
            ...
            client.setMethod("post");//登录改为post请求
            Map<String,String> params = new HashMap<String, String>();
            params.put("domain","sohu.com");
            params.put("callback","passport200007278089333261106_cb1512742508288");
            params.put("appid","1019");
            params.put("userid","XXXXXXXXXXXXXXXX");
            params.put("password","XXXXXXXXXXXXX");
            params.put("persistentcookie","0");
            client.setEntity(params);
//            client.setEntity("domain=sohu.com&callback=passport200007278089333261106_cb1512742508288&appid=1019&userid=XXXXXXXXXXXXXXXX&password=XXXXXXXXXXXXX&persistentcookie=0");
            ...
            String loginHtml = client.request("http://passport.sohu.com/apiv2/login", "utf-8");//登录搜狐网站
            System.out.println(loginHtml);

            client.setMethod("get");//登录改为post请求
            loginHtml = client.request("http://passport.sohu.com/apiv2/toolbar/userinfo?callback=jQuery112404914197299918357_1512742508303&_=1512742508445", "utf-8");//获取部分个人信息
            System.out.println(loginHtml);
        ...
        if (METHOD_NAME.equalsIgnoreCase("get")) {
            if (httpGet == null) {
                ...
                try {
                    ...
                    log.error("初始化htpclinet的get请求对象异常",e);
                }
            }
            ...
            if (httpPost == null) {
                ...
                try {
                    ...
                    log.error("初始化htpclinet的post请求对象异常",e);
                }
            }
    ...
    public void closeClient(boolean isInit) {
        if (httpclient != null) {
            try {
                ...
                log.error("关闭client连接对象释放对象并重新初始化异常，isInit:{}",isInit,e);
            } finally {
        ...
        if (httpclient == null) {
            try {
               ...
               /* .setSSLSocketFactory(new SSLConnectionSocketFactory(sslcontext))*/
                ...
                log.error("初始化clinet请求对象-并初始化HTTP/HTTPS两种协议异常",e);
            }
        }
    }
    ...
    public String request(String url, String utf) throws Exception {
        try {
            utf = StringUtils.isBlank(utf) ? "utf-8" : utf;
            HttpEntity entity = request(url);
            InputStream inputStream = entity.getContent();
            if (inputStream != null) {
    ...
    public String readLineInputStream(HttpEntity httpEntity) throws Exception {
        try {
            ...
            log.error("重载读取流异常:httpEntity:{}",httpEntity,e);
        ...
        try {
            if (isFormat) {
                ...
                if (StringUtils.isNotBlank(contentType)) {
                    ...
                    contentType = StringUtils.isNotBlank(contentType) ? contentType.substring(contentType.indexOf("/") + 1, endIndex) : "";
                }
            } else {
                return getContentType().toString();
            }
        } catch (Exception e) {
            log.error("获取响应的实体类型 并获取具体类型:jpeg/html/png/gif......异常:isFormat:{}",isFormat,e);
        }
        return contentType;
    }
    ...
    public void setHttpHost(String hostVal) throws Exception {
        if (StringUtils.isNotBlank(hostVal) && hostVal.indexOf(":") != -1) {
            try {
                String ip = hostVal.substring(0, hostVal.indexOf(":"));
                String host = hostVal.substring(hostVal.indexOf(":") + 1);
                if (StringUtils.isNotBlank(ip) && isIP(ip) && StringUtils.isNotBlank(host)) {
     ...
     * @return
     */
    public static Result getResult(String strTxt){
        Result result = Result.newSuccess();
        try {
            if(StringUtils.isBlank(strTxt)) {
                result.setMessage("返回值为空值");
            }else{
                JSONObject jo = JSON.parseObject(strTxt);
                if(jo==null||jo.keySet().size()>4){
                    result = Result.newResult(ResultEnum.FAIL,"");
                    result.setData(strTxt);
                }else{
                    result = JSON.toJavaObject(jo,Result.class);
            ...
            log.error("http返回值转换出现异常：strTxt：{}",strTxt,e);
            result = Result.newResult(ResultEnum.FAIL,"");
            result.setData(strTxt);
        }
        return result;
